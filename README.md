...
```react-dashboard```는 다음과 같은 구조로 구성되어 있습니다. 
...
react-dashboard/ 
  ...
  public/
    ...
    favicon.ico
  src/
    apis/              -> 서버와 통신하는 Promise 객체의 모음 
    components/        -> component 모음   
    containers/        -> container 모음 
    hooks/             -> 사용자 정의된 hook 모음 
    modules/           -> redux action, reducer 모음으로 ducks 패턴의 변형입니다. 
    pages/             -> pages 모음 
    routers/           -> router path를 배열로 정의했습니다.
    sagas/             -> redux saga 모음 
    stores/            -> store 객체 정의
    App.js             -> App.js 
    axios.config.js    -> 사용자 정의된 axios 객체 
    history.js         -> 사용자 정의된 router 
    index.js           -> javascript entry point 
  package.json
  README.md   
```

이 프로젝트를 빌드하기 위해서는 **아래 파일이 정확한 경로에 위치해야 합니다.**:
